...
'Original', 'Histograma', 'Thr Otsu',
'Gaussiano', 'Histograma', 'Thr Otsu']

miArray = np.arange(3)  #Creamos una matriz de 3 filas y 3 columnas

for i in miArray: #Recorremos la matriz
    plt.subplot(3,3,i*3+1), plt.imshow(imagenes[i*3], 'gray') #Creamos una subplota
    plt.title(titles[i*3]), plt.xticks([]), plt.yticks([]) #Creamos un titulo para la subplota

    plt.subplot(3,3,i*3+2), plt.hist(imagenes[i*3].ravel(), 256) #Creamos un histograma
